...
class EnhancedTable extends React.Component {
  ...
  createRow(n, no, classes) {
    ...
    var tags = (
      <div key={no}>
        <Divider />
        <Grid container onClick={event => this.selectRow(event, n)} className={[isSelected ? classes.selectedRow : classes.defaultRow, classes.tableRow]}>
          <Grid item className={[classes.cell, classes.cellNo]}>{no}</Grid>
          <Grid item className={[classes.cell, classes.cellThumb]} onClick={event => this.showThumbnailDialog(n.thumbnailPath)}>
            <Image src={n.thumbnailPath} width={60} height={60} />
          </Grid>
    ...
    if (n.version && isShowArchives) {
      var rows = n.version.map((archive, index) => {
        ...
        return (
          <div key={no + "-" + (index + 1)}>
            <Divider />
            <Grid container onClick={event => this.selectRow(event, archive)} className={[isSelected ? classes.selectedRow : classes.archiveRow, classes.tableRow]}>

              <Grid item className={[classes.cell, classes.cellArchiveNo]}>{no}{"-"}{index + 1}</Grid>
              <Grid item className={[classes.cell, classes.cellThumb]} onClick={event => this.showThumbnailDialog(archive.thumbnailPath)}>
            ...
            {({ style, isSticky }) =>
              (
                <Grid style={style} container className={isSticky ? classes.stickyHeader : classes.header} >
                  <Grid item className={[classes.cell, classes.cellNo]}>No</Grid>
                  <Grid item className={[classes.cell, classes.cellThumb]} >サムネイル</Grid>
